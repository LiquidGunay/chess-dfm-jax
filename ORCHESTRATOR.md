...
To solve this, we use a **headless orchestrator VM** running completely in the cloud (`dfm-orchestrator-vm-7` in `us-central1-a`). 
This VM runs our `sweep_manager.py` script as a persistent background daemon.

1. **Job Distribution:** The orchestrator reads our `hparam_sweep.jsonl` configuration from Google Cloud Storage (`gs://gunay-chess-experiments-us-central2/configs/hparam_sweep.jsonl`).
...
4. **Resilience:** The TPUs save checkpoint state periodically to the *same* regional bucket (pinned to `us-central2`). If an instance dies, the next provisioned TPU resumes training from the exact same step seamlessly.
...
gcloud compute tpus queued-resources list --zone=us-central2-b --project=project-b9551f07-5f68-491a-8a0

# Check US Central 1 (v5litepod-16 pods)
gcloud compute tpus queued-resources list --zone=us-central1-a --project=project-b9551f07-5f68-491a-8a0

# Check Europe West 4 (v5litepod-16 pods)
gcloud compute tpus queued-resources list --zone=europe-west4-b --project=project-b9551f07-5f68-491a-8a0
```
...
```bash
gcloud compute ssh dfm-orchestrator-vm-7 --project=project-b9551f07-5f68-491a-8a0 --zone=us-central1-a --command="sudo cat /root/sweep_hparam.log | tail -n 50"
```
...
3. Upload the snapshot to all regional buckets (e.g. overwriting `verified_source_v27_dfm.tar.gz`).

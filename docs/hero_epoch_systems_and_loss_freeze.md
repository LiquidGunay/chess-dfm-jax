...
## SIGReg sample-count decision

The separately scoped no-update audit is complete at physical batch 1,024,
using nested deterministic samples of 64, 128, and 256 examples at both fresh
initialization and the completed hero checkpoint. Both target and prediction
coefficients remained `2.0`; the audit did not optimize the model.

| State | Count-64 shared-statistic std | Count-128 std | Count-256 std | 256 reduction |
|---|---:|---:|---:|---:|
| Fresh initialization | `0.15642` | `0.13190` | `0.04759` | `69.6%` |
| Hero terminal | `0.01718` | `0.00682` | `0.00353` | `79.5%` |

The isolated statistic takes `2.321/3.494/5.554 ms` at fresh initialization
for counts `64/128/256`. Representative peak allocated HBM stays effectively
unchanged at `17.34 GB`. At initialization the count-256 shared gradient has
`0.9496` cosine and `0.9686` norm ratio against count 64. At the terminal
checkpoint the same comparison changes direction and magnitude materially;
the direct count-256 versus count-128 global cosine is `0.8709` with a
`0.7024` norm ratio. More samples therefore reduce estimator dispersion
cheaply, but they are a real objective/noise change rather than a free systems
optimization.

The coefficient stays `2.0`: the statistic is normalized by selected-example
count, and at initialization the count-256 mean shared penalty and gradient
norm are only about `3%` below count 64. The first scientific test is a
seed/data-matched fresh-initialization count-64 control versus count-256
candidate. Count 256 is not adopted until that training comparison passes.
Full-batch count 1,024 is deferred: it may remove useful stochastic
regularization and should be considered only after count 256 has positive
training evidence.

The reproducible compact decision record is
`research/analysis/hero_sigreg_sample_audit_20260727.json`, generated by
`research/analysis/build_hero_sigreg_sample_audit.py`.

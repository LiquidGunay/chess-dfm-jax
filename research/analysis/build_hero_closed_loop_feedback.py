#!/usr/bin/env python3
"""Build the immutable hero closed-loop-feedback comparison record."""
...
OUTPUT_JSON = (
    ROOT / "research/analysis/hero_closed_loop_feedback_20260728.json"
)
OUTPUT_PNG = (
    ROOT / "research/analysis/hero_closed_loop_feedback_20260728.png"
)
RUNS = {
    "control_feedback_off": "torch_autoresearch_sigreg64_u1024_v1",
    "candidate_final_pass_adjoint": (
        "torch_autoresearch_feedback_u1024_v1"
    ),
}
EVALS = {
    "control_feedback_off": (
        "torch_autoresearch_sigreg64_u1024_v1_fast_eval_v1"
    ),
    "candidate_final_pass_adjoint": (
        "torch_autoresearch_feedback_u1024_v1_fast_eval_v1"
    ),
}
ARENAS = {
    "control_feedback_off": (
    ...
    "candidate_final_pass_adjoint": (
        "autoresearch-feedback-u1024-vs-raw-bt4-128pairs-v1"
    ),
}
BASE_TRAIN_METRICS = (
    "dfm_ce_loss",
...
FEEDBACK_VALIDATION_METRICS = (
    "jepa_feedback_active",
    "jepa_feedback_applied_rms",
    "jepa_feedback_cap_fraction",
    "jepa_feedback_delta_rms",
    "jepa_feedback_dfm_ce_improvement",
...
def dictionary_diff(
    ...
    return {
        key: {
            "control_feedback_off": control.get(key),
            "candidate_final_pass_adjoint": candidate.get(key),
        }
        for key in keys
...
def metric_delta(
    ...
    for key in sorted(set(control) & set(candidate)):
        ...
        result[key] = {
            "control_feedback_off": control_value,
            "candidate_final_pass_adjoint": candidate_value,
...
def training_record(
    ...
    *,
    ...
    record = {
        ...
        "first_64_update_mean": summarize(
            rows[:64],
            BASE_TRAIN_METRICS,
        ),
        "last_64_update_mean": summarize(
            rows[-64:],
            BASE_TRAIN_METRICS,
        ),
        "terminal": {
            metric: float(rows[-1][metric])
            for metric in BASE_TRAIN_METRICS
        },
        "feedback_active": feedback_active,
    }
    if feedback_active:
        record["feedback_first_64_update_mean"] = summarize(
            rows[:64],
            FEEDBACK_TRAIN_METRICS,
        )
        record["feedback_last_64_update_mean"] = summarize(
            rows[-64:],
            FEEDBACK_TRAIN_METRICS,
        )
        record["feedback_terminal"] = {
            metric: float(rows[-1][metric])
            for metric in FEEDBACK_TRAIN_METRICS
        }
    else:
        unexpected = [
            key
            for row in rows
            for key in FEEDBACK_TRAIN_METRICS
            if key in row and float(row[key]) != 0.0
        ]
        if unexpected:
            raise ValueError(
                f"Feedback-off control has active diagnostics: {unexpected}"
            )
    return record


def feedback_validation_record(
    report: dict[str, Any],
) -> dict[str, Any]:
    record = validation_record(report)
    mode = report["config"].get("jepa_feedback_mode", "none")
    record["jepa_feedback_mode"] = mode
    record["feedback_diagnostics"] = {
        key: float(report["metrics"].get(key, 0.0))
        for key in FEEDBACK_VALIDATION_METRICS
    }
    return record
...
def build_plot(
    ...
    colors = {
        "control_feedback_off": "#31688e",
        "candidate_final_pass_adjoint": "#d1495b",
    }
    labels = {
        "control_feedback_off": "Feedback off",
        "candidate_final_pass_adjoint": "Pass-8 feedback",
    }
    ...
    candidate_rows = rows["candidate_final_pass_adjoint"]
    feedback_improvement = np.asarray(
        [
            float(row["jepa_feedback_dfm_ce_improvement"])
            for row in candidate_rows
        ],
        dtype=np.float64,
    )
    feedback_updates, feedback_smoothed = rolling_mean(
    ...
    axes[1, 0].plot(
        feedback_updates,
        feedback_smoothed,
        color=colors["candidate_final_pass_adjoint"],
        label="preliminary CE − final CE",
    )
    ...
    for axis, title, ylabel in (
        ...
        (
            axes[1, 0],
            "Feedback usefulness during training",
            "CE improvement",
        ),
    ):
        axis.axvline(
    ...
    for index, name in enumerate(names):
        ...
        axes[1, 1].annotate(
            (
                ...
                f"{arenas[name]['mean_physical_call_ms']:.1f} ms/call"
            ),
            (index, scores[index]),
            xytext=(0, 5),
    ...
    figure.suptitle(
        "Proposal-derived final-pass JEPA feedback",
        fontsize=14,
    )
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            f"Missing closed-loop-feedback inputs: {missing}"
        )
    ...
    scientific_diff = dictionary_diff(
        normalized_configs["control_feedback_off"],
        normalized_configs["candidate_final_pass_adjoint"],
    )
    if set(scientific_diff) != {"jepa_feedback_mode"}:
        raise ValueError(
            f"Unexpected scientific config differences: {scientific_diff}"
        )
    if configs["control_feedback_off"]["data"] != configs[
        "candidate_final_pass_adjoint"
    ]["data"]:
        raise ValueError("Feedback runs did not record the same data contract")
    if len(rows["control_feedback_off"]) != len(
        rows["candidate_final_pass_adjoint"]
    ):
        raise ValueError("Feedback runs have different metric-row counts")
    ...
    training = {
        name: training_record(
            ...
            rows[name],
            feedback_active=(
                name == "candidate_final_pass_adjoint"
            ),
        )
        for name in RUNS
    }
    ...
    validation = {
        name: feedback_validation_record(validation_reports[name])
        for name in RUNS
    }
    if validation["control_feedback_off"]["jepa_feedback_mode"] != "none":
        raise ValueError("Control validation unexpectedly enabled feedback")
    if (
        validation["candidate_final_pass_adjoint"][
            "jepa_feedback_mode"
        ]
        != "final_pass_adjoint"
    ):
        raise ValueError("Candidate validation did not enable feedback")
    for name in RUNS:
        ...
        if validation_reports[name]["config"]["wdl_coeff"] != 0.25:
            raise ValueError("Validation did not use WDL coefficient 0.25")
    for key in ("pool_manifest_sha256", "pool_indices_sha256", "examples"):
        if validation["control_feedback_off"][key] != validation[
            "candidate_final_pass_adjoint"
        ][key]:
    ...
    control_run_contract = dict(
        arenas["control_feedback_off"]["run_contract"]
    )
    candidate_run_contract = dict(
        arenas["candidate_final_pass_adjoint"]["run_contract"]
    )
    control_uses_jepa = control_run_contract.pop(
        "jepa_used_at_inference",
        None,
    )
    candidate_uses_jepa = candidate_run_contract.pop(
        "jepa_used_at_inference",
        None,
    )
    if control_uses_jepa is not False:
        raise ValueError("Control Arena unexpectedly used JEPA at inference")
    if candidate_uses_jepa is not True:
        raise ValueError("Candidate Arena did not use JEPA at inference")
    if control_run_contract != candidate_run_contract:
        raise ValueError("Arena run contracts are not matched")
    if (
        arenas["control_feedback_off"]["tier_contract"]
        != arenas["candidate_final_pass_adjoint"]["tier_contract"]
    ):
        raise ValueError("Arena opening tiers are not matched")
    if (
        arenas["control_feedback_off"]["reference_model_id"]
        != arenas["candidate_final_pass_adjoint"]["reference_model_id"]
    ):
        raise ValueError("Arena reference models are not matched")

    arena_delta = paired_arena_delta(
        arenas["control_feedback_off"]["_pair_scores"],
        arenas["candidate_final_pass_adjoint"]["_pair_scores"],
    )
    arena_delta["descriptive_logistic_elo_delta"] = (
        arenas["candidate_final_pass_adjoint"]["logistic_elo"]
        - arenas["control_feedback_off"]["logistic_elo"]
    )
    public_arenas = {
        name: {
            key: value
    ...
    validation_delta = metric_delta(
        validation["control_feedback_off"]["metrics"],
        validation["candidate_final_pass_adjoint"]["metrics"],
    )
    retention = rank_retention(
        validation["control_feedback_off"]["rank_diagnostics"],
        validation["candidate_final_pass_adjoint"][
            "rank_diagnostics"
        ],
    )
    candidate_feedback = validation[
        "candidate_final_pass_adjoint"
    ...
    gates = {
        ...
        "feedback_improves_candidate_validation_ce": (
            candidate_feedback[
                "jepa_feedback_dfm_ce_improvement"
            ]
            > 0.0
        ...
        "feedback_is_nonzero": (
            candidate_feedback["jepa_feedback_applied_rms"] > 0.0
        ),
        ...
        "training_throughput_at_least_200": (
            training["candidate_final_pass_adjoint"][
                "examples_per_second_end_to_end"
            ]
            >= 200.0
        ),
        "arena_call_time_within_1p5x_control": (
            public_arenas["candidate_final_pass_adjoint"][
                "mean_physical_call_ms"
            ]
            <= 1.5
            * public_arenas["control_feedback_off"][
                "mean_physical_call_ms"
            ]
        ),
    ...
    result = {
        "schema_version": "hero-closed-loop-feedback-v1",
        "question": (
            "Does proposal-derived predicted-state feedback improve "
            "the final searchless DFM refinement?"
        ),
        "comparison": {
            "control": "control_feedback_off",
            "candidate": "candidate_final_pass_adjoint",
            "only_scientific_config_difference": scientific_diff,
            "same_data_contract": True,
            "same_initialization_seed": (
                normalized_configs["control_feedback_off"]["init_seed"]
                == normalized_configs[
                    "candidate_final_pass_adjoint"
                ]["init_seed"]
                == 0
            ),
            "control_git_commit": configs[
                "control_feedback_off"
            ]["git_commit"],
            "candidate_git_commit": configs[
                "candidate_final_pass_adjoint"
            ]["git_commit"],
            "candidate_code_change_is_default_neutral": True,
            "inference_planner_calls": 8,
        },
        "training": training,
        "training_last_64_update_delta": metric_delta(
            training["control_feedback_off"]["last_64_update_mean"],
            training["candidate_final_pass_adjoint"][
                "last_64_update_mean"
            ],
        ),
        ...
        "decision": {
            "status": (
                ...
                if passes
                else "reject_final_pass_adjoint"
            ),
            "gates": gates,
            "all_preregistered_gates_pass": passes,
            "active_jepa_feedback_mode": (
                "final_pass_adjoint" if passes else "none"
            ),
            "causal_scope": (
                "One deterministic matched 3.70%-epoch screen. A positive "
                "result still requires a fresh repeat and at least 10% of "
                "an epoch; a negative result closes only the parameter-free "
                "final-pass adjoint mechanism."
            ),
        },
        "plot": {
        ...
        json.dumps(
            {
                ...
                "feedback_validation_ce_improvement": candidate_feedback[
                    "jepa_feedback_dfm_ce_improvement"
                ],

...
class TransformerStack(nn.Module):
    ...
    def _qk_norm(x: Tensor, compute_dtype: torch.dtype) -> Tensor:
        stats = x.float()
        return (
            stats * torch.rsqrt(stats.square().mean(dim=-1, keepdim=True) + 1e-6)
        ).to(compute_dtype)
    ...
    def _layer(self, x: Tensor, layer: int, compute_dtype: torch.dtype) -> Tensor:
        ...
        if self.use_xsa:
            value = v.float()
            value_direction = value * torch.rsqrt(
...
class StateProjector(nn.Module):
    ...
    def forward(self, tokens: Tensor, compute_dtype: torch.dtype) -> Tensor:
        ...
        squares = (
            tokens.to(compute_dtype).reshape(batch * 64, 1024)
            @ self.in_proj.to(compute_dtype)
...
class JointModel(nn.Module):
    def __init__(self, config: Config = CONFIG):
        ...
        self.jepa_action_embed = RawEmbedding(
            _VOCAB_SIZE + 1, config.z_dim, dtype=dtype
        )
        self.jepa_hidden_adapter = RawLinear(
            config.token_dim, config.z_dim, dtype=dtype
        )
    ...
    def encode_selected(
        ...
        z_all = self.state_projector(
            tokens.reshape(batch * 2, 64, 1024), compute_dtype
...
def loss_and_aux(
    ...
    *,
    ...
    z_all, z_dfm = model.encode_selected(
        batch["current_planes"], selected_planes, compute_dtype
    )
    z_jepa = z_all[:, 0]
    target_z = z_all[:, 1:]
    ...
    noisy_actions = torch.where(
        is_masked, torch.full_like(actions, _MASK_TOKEN), actions
    )
    ...
    dfm_ce = (ce_by_horizon * active_horizon).sum() / active_horizon.sum().clamp_min(
        1.0
    )
    ...
    clean_result = model.planner(
        z_dfm, actions, clean_t, compute_dtype, return_hidden=True
    )
    ...
    target_weight = torch.cat(
        (sigreg_valid.unsqueeze(1), target_future_weight), dim=1
    ).reshape(-1)
    ...
    clip_scale = torch.minimum(
        ...
        torch.as_tensor(
            config.loss_clip_value, device=stopped.device, dtype=stopped.dtype
        )
        / stopped,
    )
        ...
        for leaf in self.leaves:
            ...
            if leaf.use_muon:
                ...
                update = (
                    ...
                    + (1.0 - beta_muon) * corrected_gradient
                )
    ...
    def find_class(self, module: str, name: str) -> Any:
        ...
        if allowed is None:
            raise pickle.UnpicklingError(
                f"Forbidden global in source model: {module}.{name}"
            )
        return allowed
...
def load_verified_source_model(path: Path = _SOURCE_STATE) -> dict[str, Any]:
    ...
    with source.open("rb") as handle:
        ...
        if source_stat.st_size != _SOURCE_SIZE_BYTES:
            raise ValueError(
                f"Source size drift: {source_stat.st_size} != {_SOURCE_SIZE_BYTES}"
            )
...
def bind_source_model(
    model: JointModel, source_tree: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    if len(records) != _EXPECTED_MODEL_LEAVES:
        raise ValueError(
            f"Source leaf count drift: {len(records)} != {_EXPECTED_MODEL_LEAVES}"
        )
    if total_bytes != _EXPECTED_MODEL_BYTES:
        raise ValueError(
            f"Source model bytes drift: {total_bytes} != {_EXPECTED_MODEL_BYTES}"
        )
    return {
...
def inspect_source(args: argparse.Namespace) -> int:
    model, mapping = load_source_model(
        device=torch.device("cpu"), source_path=args.source_state
    )
